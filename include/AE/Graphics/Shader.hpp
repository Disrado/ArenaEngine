...
public:
    ...
    ///
    /// This overload can also be called with ae::Color objects
    /// that are converted to ae::Glsl::Vec4.
    ///
    /// It is important to note that the components of the color are
    /// normalized before being passed to the shader. Therefore,
    /// they are converted from range [0 .. 255] to range [0 .. 1].
    /// For example, a ae::Color(255, 127, 0, 255) will be transformed
    ...
    ///
    /// This overload can also be called with ae::Color objects
    /// that are converted to ae::Glsl::Ivec4.
    ///
    /// If color conversions are used, the ivec4 uniform in GLSL
    /// will hold the same values as the original ae::Color
    /// instance. For example, ae::Color(255, 127, 0, 255) is
    ...
    /// ae::Shader::CurrentTexture:
    /// \code
    /// shader.setUniform("the_texture", ae::Shader::CurrentTexture).
    /// \endcode
    ///
    ...
    /// \endcode
    /// \code
    /// shader.setUniform("current", ae::Shader::CurrentTexture);
    /// \endcode
    ///
    /// \param name Name of the texture in the shader
    ///
    ...
    /// mix ae::Shader with OpenGL code.
    ///
    /// \code
    /// ae::Shader s1, s2;
    /// ...
    ...
    /// any attempt to use ae::Shader will fail.
    ///
    ...
    /// any attempt to use ae::Shader geometry shader features will fail.
    ///
...
///
/// A ae::Shader can be composed of either a vertex shader
...
/// ae::Shader handles different types of uniforms:
...
/// \li ae::Color as a 4D vector (\p vec4)
/// \li ae::Transform as matrices (\p mat3 or \p mat4)
///
...
/// defined in the ae::Glsl namespace:
/// \code
/// shader.setUniform("offset", 2.f);
/// shader.setUniform("point", ae::Vector3f(0.5f, 0.8f, 0.3f));
/// shader.setUniform("color", ae::Glsl::Vec4(color));          // color is a ae::Color
/// shader.setUniform("matrix", ae::Glsl::Mat4(transform));     // transform is a ae::Transform
/// shader.setUniform("overlay", texture);                      // texture is a ae::Texture
/// shader.setUniform("current", ae::Shader::CurrentTexture);
/// \endcode
///
...
/// not interesting. For example, using a vertex shader on a ae::Sprite
/// is limited because there are only 4 vertices, the sprite would
/// have to be subdivided in order to apply wave effects.
/// Another bad example is a fragment shader with ae::Text: the texture
...
/// current contents of the target (like the old ae::PostFx class
/// in SFML 1). This can be done in two different ways:
/// \li draw everything to a ae::RenderTexture, then draw it to
///     the main target using the shader
/// \li draw everything directly to the main target, then use
///     ae::Texture::update(Window&) to copy its contents to a texture
...
///
/// Like ae::Texture that can be used as a raw OpenGL texture,
/// ae::Shader can also be used directly as a raw shader for
/// custom OpenGL geometry.
/// \code
/// ae::Shader::bind(&shader);
