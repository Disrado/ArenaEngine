...
// Headers
////////////////////////////////////////////////////////////
#include <AE/Window/JoystickImpl.hpp>
...
namespace
{
    ...
    const ae::Time connectionRefreshDelay = ae::milliseconds(500);

    ConnectionCache connectionCache[ae::Joystick::Count];
    ...
    std::string getErrorString(DWORD error)
    {
        ...
        ae::String message = buffer;
        LocalFree(buffer);
        return message.toAnsiString();
    }

    // Get the joystick's name
    ae::String getDeviceName(unsigned int index, JOYCAPS caps)
    {
        // Give the joystick a default name
        ae::String joystickDescription = "Unknown Joystick";
        ...
        if (result != ERROR_SUCCESS)
        {
            ...
            if (result != ERROR_SUCCESS)
            {
                ae::err() << "Unable to open registry for joystick at index " << index << ": " << getErrorString(result) << std::endl;
        ...
        if (result != ERROR_SUCCESS)
        {
            ae::err() << "Unable to query registry key for joystick at index " << index << ": " << getErrorString(result) << std::endl;
            return joystickDescription;
        }
        ...
        if (result != ERROR_SUCCESS)
        {
            ae::err() << "Unable to open registry key for joystick at index " << index << ": " << getErrorString(result) << std::endl;
            return joystickDescription;
        }
        ...
        if (result != ERROR_SUCCESS)
        {
            ae::err() << "Unable to query name for joystick at index " << index << ": " << getErrorString(result) << std::endl;
            return joystickDescription;
        }
