...
namespace
{
    ...
    void updatePluggedList(udev_device* udevDevice = NULL)
    {
                ...
                if (isJoystick(udevDevice))
                {
                    ...
                    if (record == joystickList.end())
                    {
                        ...
                        else if (std::strstr(action, "remove"))
                        {
                            ...
                            ae::err() << "Trying to disconnect joystick that wasn't connected" << std::endl;
                        }
                    }
                }
        ...
        if (!udevEnumerator)
        {
            ae::err() << "Error while creating udev enumerator" << std::endl;
            return;
        }
        ...
        if (result < 0)
        {
            ae::err() << "Error while adding udev enumerator match" << std::endl;
            return;
        }
        ...
        if (result < 0)
        {
            ae::err() << "Error while enumerating udev devices" << std::endl;
            return;
        }
    ...
    unsigned int getJoystickVendorId(unsigned int index)
    {
        if (!udevContext)
        {
            ae::err() << "Failed to get vendor ID of joystick " << joystickList[index].deviceNode << std::endl;
            return 0;
        }
    ...
    unsigned int getJoystickProductId(unsigned int index)
    {
        if (!udevContext)
        {
            ae::err() << "Failed to get product ID of joystick " << joystickList[index].deviceNode << std::endl;
            return 0;
        }
    ...
    std::string getJoystickName(unsigned int index)
    {
        ...
        ae::err() << "Unable to get name for joystick " << devnode << std::endl;
...
void JoystickImpl::initialize()
{
    ...
    if (!udevContext)
    {
        ae::err() << "Failed to create udev context, joystick support not available" << std::endl;
        return;
    }
