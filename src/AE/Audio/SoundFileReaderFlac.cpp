...
namespace
{
    ...
    {
        ae::priv::SoundFileReaderFlac::ClientData* data = static_cast<ae::priv::SoundFileReaderFlac::ClientData*>(clientData);

        ae::Int64 count = data->stream->read(buffer, *bytes);
        if (count > 0)
        {
    ...
    FLAC__StreamDecoderSeekStatus streamSeek(const FLAC__StreamDecoder*, FLAC__uint64 absoluteByteOffset, void* clientData)
    {
        ...
        ae::Int64 position = data->stream->seek(absoluteByteOffset);
        if (position >= 0)
    ...
    FLAC__StreamDecoderTellStatus streamTell(const FLAC__StreamDecoder*, FLAC__uint64* absoluteByteOffset, void* clientData)
    {
        ...
        ae::Int64 position = data->stream->tell();
        if (position >= 0)
        {
    ...
    FLAC__StreamDecoderLengthStatus streamLength(const FLAC__StreamDecoder*, FLAC__uint64* streamLength, void* clientData)
    {
        ...
        ae::Int64 count = data->stream->getSize();
        if (count >= 0)
        {
            *streamLength = count;
