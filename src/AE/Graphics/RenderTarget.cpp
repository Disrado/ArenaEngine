...
namespace
{
    // Convert an ae::BlendMode::Factor constant to the corresponding OpenGL constant.
    ae::Uint32 factorToGlConstant(ae::BlendMode::Factor blendFactor)
    {
        switch (blendFactor)
        {
            case ae::BlendMode::Zero:             return GL_ZERO;
            case ae::BlendMode::One:              return GL_ONE;
            case ae::BlendMode::SrcColor:         return GL_SRC_COLOR;
            case ae::BlendMode::OneMinusSrcColor: return GL_ONE_MINUS_SRC_COLOR;
            case ae::BlendMode::DstColor:         return GL_DST_COLOR;
            case ae::BlendMode::OneMinusDstColor: return GL_ONE_MINUS_DST_COLOR;
            case ae::BlendMode::SrcAlpha:         return GL_SRC_ALPHA;
            case ae::BlendMode::OneMinusSrcAlpha: return GL_ONE_MINUS_SRC_ALPHA;
            case ae::BlendMode::DstAlpha:         return GL_DST_ALPHA;
            case ae::BlendMode::OneMinusDstAlpha: return GL_ONE_MINUS_DST_ALPHA;
        }

        ae::err() << "Invalid value for ae::BlendMode::Factor! Fallback to ae::BlendMode::Zero." << std::endl;
        assert(false);
        return GL_ZERO;
    }


    // Convert an ae::BlendMode::BlendEquation constant to the corresponding OpenGL constant.
    ae::Uint32 equationToGlConstant(ae::BlendMode::Equation blendEquation)
    {
        switch (blendEquation)
        {
            case ae::BlendMode::Add:             return GLEXT_GL_FUNC_ADD;
            case ae::BlendMode::Subtract:        return GLEXT_GL_FUNC_SUBTRACT;
            case ae::BlendMode::ReverseSubtract: return GLEXT_GL_FUNC_REVERSE_SUBTRACT;
        }

        ae::err() << "Invalid value for ae::BlendMode::Equation! Fallback to ae::BlendMode::Add." << std::endl;
        assert(false);
                        ...
                        PrimitiveType type, const RenderStates& states)
{
    ...
    // GL_QUADS is unavailable on OpenGL ES
        ...
        if (type == Quads)
        {
            err() << "ae::Quads primitive type is not supported on OpenGL ES platforms, drawing skipped" << std::endl;
            return;
        }
        #define GL_QUADS 0
    #endif
...
//   is not enough; if the ae::Texture instance is destroyed,
