...
#include <AE/Graphics/TextureSaver.hpp>
...
{
    ae::Mutex idMutex;
    ae::Mutex maximumSizeMutex;

    // Thread-safe unique identifier generator,
    // is used for states cache (see RenderTarget)
    ae::Uint64 getUniqueId()
    {
        ae::Lock lock(idMutex);

        static ae::Uint64 id = 1; // start at 1, zero is "no texture"
...
void Texture::bind(const Texture* texture, CoordinateType coordinateType)
{
    ...
    if (texture && texture->m_texture)
    {
        ...
        if ((coordinateType == Pixels) || texture->m_pixelsFlipped)
        {
            ...
            // Go back to model-view mode (ae::RenderTarget relies on it)
