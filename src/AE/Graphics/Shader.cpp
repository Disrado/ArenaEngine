...
namespace
{
    ae::Mutex maxTextureUnitsMutex;
    ae::Mutex isAvailableMutex;
    ...
    GLint getMaxTextureUnits()
    {
        ...
        ae::Lock lock(maxTextureUnitsMutex);
    ...
    bool getStreamContents(ae::InputStream& stream, std::vector<char>& buffer)
    {
        bool success = true;
        ae::Int64 size = stream.getSize();
        if (size > 0)
        {
            buffer.resize(static_cast<std::size_t>(size));
            stream.seek(0);
            ae::Int64 read = stream.read(&buffer[0], size);
            success = (read == size);
        }
        buffer.push_back('\0');
        return success;
    ...
    template <typename T>
    std::vector<T> flatten(const ae::Vector2<T>* vectorArray, std::size_t length)
    {
    ...
    template <typename T>
    std::vector<T> flatten(const ae::Vector3<T>* vectorArray, std::size_t length)
    {
    ...
    template <typename T>
    std::vector<T> flatten(const ae::priv::Vector4<T>* vectorArray, std::size_t length)
    {
...
struct Shader::UniformBinder : private NonCopyable
{
    ...
    GLEXT_GLhandle       currentProgram; ///< Handle to the program object of the modified ae::Shader instance
    GLint                location;       ///< Uniform location, used by the surrounding ae::Shader code
};
